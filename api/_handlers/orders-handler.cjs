...
function makeOrder(body, items, source, entrega) { const subtotal = money(items.reduce((sum, item) => sum + item.subtotal, 0)); const envio = money(entrega && entrega.costo); const total = money(subtotal + envio); const now = new Date().toISOString(); return { id: `ord_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, customerName: cleanText(body.customerName || body.name, 120), phone: cleanPhone(body.phone || body.customerPhone), products: items, subtotal, shipping: envio, delivery: entrega || { modo: 'coordinado', zonaId: null, zonaNombre: '', direccion: '', costo: 0 }, total, status: 'pending', paymentStatus: 'pending', shippingStatus: 'pending', notes: cleanText(body.notes, 1000), source, inventoryDeducted: false, createdAt: now, updatedAt: now }; }
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      next.subtotal = money(next.products.reduce((sum, item) => sum + item.subtotal, 0)); next.total = money(next.subtotal + money(next.shipping));
