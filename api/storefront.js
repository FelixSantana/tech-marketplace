...
(function(){
  ...
  function parseMessage(url){
    try{
      const u=new URL(url,location.href); const text=u.searchParams.get('text')||''; const lines=text.split(/\\n/).map(x=>x.trim()).filter(Boolean);
      const products=[]; let total=0;
      lines.forEach(line=>{
        const m=line.match(/^•\\s*(.*?)\\s+x(\\d+)\\s+—\\s+.*?([\\d.,]+)\\s*$/);
        if(m){products.push({name:m[1].trim(),quantity:Number(m[2])||1,unitPrice:Number(m[3].replace(/,/g,''))||0});}
        const t=line.match(/^Total:\\s+.*?([\\d.,]+)\\s*$/); if(t) total=Number(t[1].replace(/,/g,''))||total;
      });
      if(!products.length){
        const p=[]; for(const line of lines){if(/^•\\s*/.test(line)&&!/^•\\s*Categoría:/i.test(line)){const m=line.match(/^•\\s*(.*?)\\s*$/);if(m)p.push({name:m[1],quantity:1,unitPrice:0});}}
        products.push(...p);
      }
      if(!total) total=products.reduce((s,p)=>s+p.quantity*p.unitPrice,0);
      return {text,products,total};
    }catch(e){return {text:'',products:[],total:0};}
  }
  function openCheckout(url){
    ...
    const name=overlay.querySelector('#soName'), phone=overlay.querySelector('#soPhone'), send=overlay.querySelector('#soSend'), error=overlay.querySelector('#soError');
    name.focus();
    function close(){overlay.remove()}
    overlay.querySelector('#soCancel').onclick=close;
    overlay.addEventListener('click',e=>{if(e.target===overlay)close()});
    send.onclick=async()=>{
      const customerName=name.value.trim(); const customerPhone=phone.value.replace(/\\D/g,'');
      ...
      if(!parsed.products.length){error.textContent='No pudimos identificar los productos del pedido. Recarga la página e inténtalo de nuevo.';error.style.display='block';return;}
      send.disabled=true;send.textContent='Registrando pedido…';
      try{
        const r=await fetch('/api/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'whatsapp_checkout',customerName,phone:customerPhone,products:parsed.products,total:parsed.total,notes:'Pedido generado desde el catálogo y enviado a WhatsApp.'})});
        const data=await r.json().catch(()=>({}));
        if(!r.ok) throw new Error(data.message||'No se pudo registrar el pedido.');
        close(); window.open(url,'_blank','noopener');
      }catch(e){send.disabled=false;send.textContent='Registrar y abrir WhatsApp';error.textContent=e.message||'No se pudo registrar el pedido. Inténtalo de nuevo.';error.style.display='block'}
    };
  }
  document.addEventListener('click',function(e){
    const link=e.target.closest('a[href*="wa.me/"]');
    if(!link || link.classList.contains('disabled') || link.getAttribute('href')==='#') return;
    e.preventDefault(); e.stopImmediatePropagation(); openCheckout(link.href);
  },true);
})();
</script>`;
