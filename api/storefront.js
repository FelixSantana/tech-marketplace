...
(function(){
  ...
  function parseMessage(url){
    try{
      ...
      for(const line of lines){
        const match = line.match(/^•\s*(.*?)\s+x(\d+)\s+—\s*(?:RD\$\s*)?([\d.,]+)/i);
        if(match){
          const rawPrice = match[3].replace(/\./g,'').replace(/,(?=\d{3}(?:\D|$))/g,'');
          products.push({
            name: match[1].trim(),
        ...
        const totalMatch = line.match(/^Total:\s*(?:RD\$\s*)?([\d.,]+)/i);
        if(totalMatch){
          const rawTotal = totalMatch[1].replace(/\./g,'').replace(/,(?=\d{3}(?:\D|$))/g,'');
          total = Number(rawTotal) || total;
        }
      }
  ...
  function openCheckout(url){
    ...
    send.onclick = async ()=>{
      ...
      const customerPhone = phone.value.replace(/\D/g,'');
      ...
      try{
        const response = await fetch('/api/orders', {
          ...
          body:JSON.stringify({
            ...
            total:parsed.total,
            notes:'Pedido generado desde el catálogo y enviado a WhatsApp.'
          })
        });
