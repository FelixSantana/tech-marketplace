...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'setup') {
      ...
      if (existing) {
        return res.status(409).json({ error: 'ALREADY_SETUP', message: 'Ya existe una cuenta de administrador.' });
      }
      const email = String(body.email || '').trim().toLowerCase();
      const password = String(body.password || '');
      if (!email || email.indexOf('@') === -1) {
        return res.status(400).json({ error: 'BAD_EMAIL', message: 'Ingresa un correo válido.' });
      }
      if (password.length < 6) {
        return res.status(400).json({ error: 'BAD_PASSWORD', message: 'La contraseña debe tener al menos 6 caracteres.' });
      }
      const salt = crypto.randomBytes(16).toString('hex');
      const hash = hashPassword(password, salt);
      const secret = crypto.randomBytes(32).toString('hex');
      const admin = { email, salt, hash, secret };
    ...
    if (action === 'login') {
      ...
      if (!admin) {
        return res.status(400).json({ error: 'NOT_SETUP', message: 'Aún no se ha creado una cuenta de administrador.' });
      }
      const email = String(body.email || '').trim().toLowerCase();
      const password = String(body.password || '');
    ...
    if (action === 'change') {
      ...
      let changed = false;
      if (body.newEmail && String(body.newEmail).trim()) {
      ...
      if (body.newPassword && String(body.newPassword).trim()) {
        ...
        admin.salt = newSalt;
        admin.hash = hashPassword(newPassword, newSalt);
        changed = true;
      }
      if (!changed) {
        return res.status(400).json({ error: 'NOTHING_TO_CHANGE', message: 'No hay cambios para guardar.' });
      }
    ...
    return res.status(400).json({ error: 'BAD_ACTION', message: 'Acción no reconocida.' });
  } catch (e) {
