...
export default function App() {
  ...
  return <div id="app"><Header settings={settings} theme={theme} onToggleTheme={() => setTheme((t) => t === 'light' ? 'dark' : 'light')} searchTerm={searchTerm} setSearchTerm={setSearchTerm} /><CategoryChips products={products} activeCategory={activeCategory} setActiveCategory={setActiveCategory} /><main>{loadError && !loading ? <div className="empty-state" style={{ padding: '70px 20px' }}><div className="glyph">🔌</div><h3>No se pudo conectar con el catálogo</h3><p>{loadError}</p></div> : <ProductGrid products={disponibles} allProducts={disponibles} settings={settings} activeCategory={activeCategory} searchTerm={searchTerm} onOpenDetail={setDetailProductId} onAddCart={handleAddCart} onOrderWhatsApp={handleSingleOrder} />}</main><footer>Catálogo digital de {settings.storeName || 'Synaptic Tech'}</footer>
    ...
    <Toast message={message} visible={visible} />{detailProductId && <ProductDetail product={disponibles.find((p) => p.id === detailProductId)} settings={settings} onClose={() => setDetailProductId(null)} onAddCart={(id, qty, variantId) => handleAddCart(id, qty, variantId)} onOrderWhatsApp={handleSingleOrder} />}{modal === 'cart' && <CartModal cart={cart} products={disponibles} settings={settings} onClose={() => setModal(null)} onUpdateQty={updateCartQty} onRemove={removeFromCart} onClear={clearCart} onCheckout={handleCartCheckout} showToast={showToast} />}{modal === 'checkout' && <CheckoutModal items={checkoutItems} settings={settings} onClose={() => { setModal(null); setCheckoutItems([]); setCheckoutFromCart(false); }} onOrderCreated={() => { if (checkoutFromCart) clearCart(); showToast('Pedido registrado correctamente'); }} showToast={showToast} />}{modal === 'setup' && <SetupModal authRequest={authRequest} setAdminToken={setAdminToken} showToast={showToast} onGoLogin={() => setModal('login')} onSetupComplete={handleSetupComplete} />}{modal === 'login' && <LoginModal authRequest={authRequest} setAdminToken={setAdminToken} onLoginSuccess={() => setModal('admin')} onClose={() => setModal(null)} showToast={showToast} />}{modal === 'admin' && <AdminPanel products={products} reservas={reservas} categories={categories} setCategories={setCategories} settings={settings} saveCatalog={saveCatalog} refreshCatalog={() => fetchCatalog({ silent: true })} adminToken={adminToken} authRequest={authRequest} setAdminToken={setAdminToken} onClose={() => setModal(null)} onLogout={handleLogout} showToast={showToast} />}</div>;
}
