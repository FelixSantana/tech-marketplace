...
export default function App() {
  ...
  const handleOrderCreated = () => showToast('Pedido registrado correctamente');
  ...
  return <div id="app">
    ...
    {settings.configured && isAdminRoute && <button className="admin-fab admin-settings-fab" onClick={openAdmin} title="Panel de administrador" aria-label="Panel de administrador"><svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3.5l1.1 1.8a7.3 7.3 0 0 1 1.7.7l2-.6 1.6 1.6-.6 2a7.3 7.3 0 0 1 .7 1.7l1.8 1.1v2.4l-1.8 1.1a7.3 7.3 0 0 1-.7 1.7l.6 2-1.6 1.6-2-.6a7.3 7.3 0 0 1-1.7.7L12 20.5l-2.4-.1-1.1-1.8a7.3 7.3 0 0 1-1.7-.7l-2 .6-1.6-1.6.6-2a7.3 7.3 0 0 1-.7-1.7L1.3 12l1.8-1.1a7.3 7.3 0 0 1 .7-1.7l-.6-2 1.6-1.6 2 .6a7.3 7.3 0 0 1 1.7-.7L9.6 3.4z"/><circle cx="12" cy="12" r="2.7"/></svg></button>}
    <Toast message={message} visible={visible} />
    {detailProductId && <ProductDetail product={products.find((p) => p.id === detailProductId)} settings={settings} onClose={() => setDetailProductId(null)} onAddCart={(id, qty) => handleAddCart(id, qty)} onOrderWhatsApp={handleSingleOrder} />}
    {modal === 'cart' && <CartModal cart={cart} products={products} settings={settings} onClose={() => setModal(null)} onUpdateQty={updateCartQty} onRemove={removeFromCart} onClear={clearCart} onCheckout={handleCartCheckout} showToast={showToast} />}
    {modal === 'checkout' && <CheckoutModal items={checkoutItems} settings={settings} onClose={() => { setModal(null); setCheckoutItems([]); }} onOrderCreated={handleOrderCreated} showToast={showToast} />}
    {modal === 'setup' && <SetupModal settings={settings} authRequest={authRequest} setAdminToken={setAdminToken} showToast={showToast} onSetupComplete={async (updates) => { const nextSettings = { ...settings, ...updates }; setSettings(nextSettings); await saveCatalog(adminToken, { settings: nextSettings }); showToast('Configuración guardada'); setModal('admin'); }} />}
    {modal === 'login' && <LoginModal authRequest={authRequest} setAdminToken={setAdminToken} onLoginSuccess={() => setModal('admin')} onClose={() => setModal(null)} showToast={showToast} />}
    {modal === 'admin' && <AdminPanel products={products} setProducts={setProducts} categories={categories} setCategories={setCategories} settings={settings} setSettings={setSettings} saveCatalog={saveCatalog} adminToken={adminToken} authRequest={authRequest} setAdminToken={setAdminToken} onClose={() => setModal(null)} onLogout={handleLogout} showToast={showToast} />}
  </div>;
}
