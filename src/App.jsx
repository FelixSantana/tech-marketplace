...
export default function App() {
  ...
  const handleCartCheckout = useCallback(() => { const items = cart.map((ci) => { const product = products.find((p) => p.id === ci.productId); return product ? { product, qty: ci.qty } : null; }).filter(Boolean); if (!items.length) return showToast('Tu carrito está vacío.'); setModal(null); startCheckout(items, true); }, [cart, products, startCheckout, showToast]);
