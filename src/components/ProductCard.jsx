    ...
    <div className="card" onClick={() => onOpenDetail(product.id)}>
      ...
      <div className="card-body">
        ...
        <div className="card-actions">
          <button className="btn-cart-add" title="Agregar al carrito" disabled={isOut} onClick={(e) => { e.stopPropagation(); if (conVariantes) return openDetail(); onAddCart(product.id, 1); }}>
          ...
          <button className={`btn-wa ${isOut || (!conVariantes && !settings.whatsapp) ? 'disabled' : ''}`} disabled={isOut || (!conVariantes && !settings.whatsapp)} onClick={(e) => { e.stopPropagation(); if (conVariantes) return openDetail(); onOrderWhatsApp(product, 1); }} title={conVariantes ? 'Ver opciones disponibles' : 'Pedir por WhatsApp'}>
            ...
            {isOut ? 'Agotado' : conVariantes ? 'Ver opciones' : settings.whatsapp ? 'Pedir' : 'Configurar'}
          </button>
        </div>
      </div>
    </div>
