...
export default function ProductCard({ product, settings, onOpenDetail, onAddCart }) {
  const stockQty = getStockQty(product);
  const isOut = stockQty <= 0;
  const waLink = settings.whatsapp ? buildWaLink(product, 1, settings) : '#';
  const disabledClass = settings.whatsapp ? '' : 'disabled';
    ...
    <div className="card" onClick={() => onOpenDetail(product.id)}>
      ...
      <div className="card-body">
        ...
        <div className="card-actions">
          ...
          <a className={`btn-wa ${isOut ? 'disabled' : disabledClass}`} href={isOut ? '#' : waLink} target="_blank" rel="noopener noreferrer" title="Pedir por WhatsApp" onClick={(e) => e.stopPropagation()}>
