...
import { buildCartWaLink } from '../lib/utils';

export default function CartModal({ cart, products, settings, onClose, onUpdateQty, onRemove, onClear, showToast }) {
        ...
        {items.length === 0 ? (
          ...
          <>
              ...
              {items.map((it) => {
                ...
                return (
                  <div className="cart-row" key={it.productId}>
                    ...
                    <div className="info">
                      <div className="n">{it.product.name}</div>
                      <div className="p mono">{settings.currency} {Number(it.product.price).toLocaleString('es-DO')} c/u</div>
                    </div>
                    <div className="qty-stepper" style={{ flexShrink: 0 }}>
                      <button disabled={it.qty <= 1} onClick={() => onUpdateQty(it.productId, it.qty - 1)}>−</button>
                      <span className="qty-val">{it.qty}</span>
                      <button disabled={it.qty >= stockQty} onClick={() => onUpdateQty(it.productId, it.qty + 1)}>+</button>
                    </div>
            ...
            <div className="cart-total-row">
              <span>Total</span>
              <span className="mono">{settings.currency} {total.toLocaleString('es-DO')}</span>
            </div>
            <button className="btn-primary" onClick={() => {
              if (!settings.whatsapp) { showToast('Configura primero el WhatsApp de la tienda'); return; }
              window.open(buildCartWaLink(items, settings), '_blank');
            }}>Finalizar pedido por WhatsApp</button>
