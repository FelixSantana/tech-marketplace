    ...
    try {
      const response = await fetch('/api/orders.cjs', {
        ...
        body: JSON.stringify({
          ...
          phone: cleanPhone,
          notes,
          products: items.map((item) => ({ productId: item.product.id, quantity: item.qty }))
        })
      });
      ...
      items.forEach((item) => {
        ...
        lines.push(`• ${item.product.name} x${item.qty} — ${settings.currency} ${subtotal.toLocaleString('es-DO')}`);
      });
    ...
    <div className="overlay" onClick={(e) => { if (e.target.classList.contains('overlay') && !saving) onClose(); }}>
      <form className="panel" style={{ maxWidth: 520 }} onSubmit={submit}>
        ...
        <div className="form-actions" style={{ marginTop: 16 }}>
          ...
          <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Registrando…' : 'Registrar y abrir WhatsApp'}</button>
        </div>
      </form>
    </div>
  );
