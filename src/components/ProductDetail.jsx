...
export default function ProductDetail({ product, settings, onClose, onAddCart }) {
  ...
  const waLink = settings.whatsapp && !isOut ? buildWaLink(product, qty, settings) : '#';
        ...
        <div className="detail-body">
          ...
          {!isOut && (
            <div className="qty-row">
              <span className="qlabel">Cantidad a pedir</span>
              <div className="qty-stepper">
                <button disabled={qty <= 1} onClick={() => setQty((q) => Math.max(1, q - 1))}>−</button>
                <span className="qty-val">{qty}</span>
                <button disabled={qty >= stockQty} onClick={() => setQty((q) => Math.min(stockQty, q + 1))}>+</button>
              </div>
            </div>
          )}
          {!isOut && (
            <button className="btn-primary" onClick={() => onAddCart(product.id, qty)}>Agregar {qty} al carrito</button>
          )}
          <a className={`btn-wa ${isOut ? 'disabled' : ''}`} href={waLink} target="_blank" rel="noopener noreferrer" style={{ marginTop: 8 }} title="Pedir solo este producto por WhatsApp">
            ...
            {isOut ? 'Agotado' : settings.whatsapp ? 'Pedir solo esto por WhatsApp' : 'Configura tu WhatsApp'}
          </a>
          {!isOut && settings.whatsapp && (
            <div className="detail-total">Total: {settings.currency} {(Number(product.price) * qty).toLocaleString('es-DO')}</div>
          )}
        </div>
      </div>
    </div>
