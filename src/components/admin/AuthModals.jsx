...
export function SetupModal({ authRequest, setAdminToken, onSetupComplete, showToast, onGoLogin }) {
  ...
  return <div className="overlay"><div className="panel login-panel setup-panel"><div className="login-badge"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 3l8 4v5c0 5-3.5 8.5-8 9-4.5-.5-8-4-8-9V7l8-4z"/><path d="M9 12l2 2 4-4"/></svg></div><h2 className="login-title">Crear administrador</h2><p className="hint" style={{ marginBottom: 18 }}>Crea primero el acceso seguro. La configuración de la tienda se realiza después desde el panel.</p><div className="field"><label htmlFor={`${uid}-email`}>Correo de administrador</label><div className="input-with-icon"><span>✉</span><input id={`${uid}-email`} type="email" placeholder="tucorreo@ejemplo.com" autoComplete="username" autoFocus value={email} onChange={(e) => setEmail(e.target.value)} /></div></div><div className="field"><label htmlFor={`${uid}-password`}>Contraseña</label><input id={`${uid}-password`} type="password" placeholder="Mínimo 6 caracteres" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} /></div><div className="field"><label htmlFor={`${uid}-confirm`}>Confirmar contraseña</label><input id={`${uid}-confirm`} type="password" placeholder="Repite la contraseña" autoComplete="new-password" value={confirm} onChange={(e) => setConfirm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSave()} /></div><button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Creando cuenta…' : 'Crear administrador'}</button>{onGoLogin && <button type="button" className="btn-secondary auth-switch" onClick={onGoLogin}>Ya tengo una cuenta · Iniciar sesión</button>}</div></div>;
}

export function LoginModal({ authRequest, setAdminToken, onLoginSuccess, onClose, showToast }) {
  const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [saving, setSaving] = useState(false); const uid = useId();
